


EKF_diff_with_gps = EKF_x_with_gps(1:3,:,:,:) - repmat(reshape(x_truth(1:3,gps_idxs,:), [3, nCars_with_gps, 1, nTicks]), [1, 1, nSims, 1]);
EKF_err_with_gps  = [sqrt(EKF_diff_with_gps(1,:,:,:).^2 + EKF_diff_with_gps(2,:,:,:).^2); EKF_diff_with_gps(3,:,:,:,:)];
EKF_rmse_with_gps = reshape(RMSE(EKF_err_with_gps), [2, nSims]);


EKF_diff_no_gps = EKF_x_no_gps(1:3,:,:,:) - repmat(reshape(x_truth(1:3,no_gps_idx,:), [3, nCars_no_gps, 1, nTicks]), [1, 1, nSims, 1]);
EKF_err_no_gps  = [sqrt(EKF_diff_no_gps(1,:,:,:).^2 + EKF_diff_no_gps(2,:,:,:).^2); EKF_diff_no_gps(3,:,:,:,:)];
EKF_rmse_no_gps = reshape(RMSE(EKF_err_no_gps), [2, nSims]);  



DCL_diff = DCL_x(1:3,:,:,:) - repmat(reshape(x_truth(1:3,:,:), [3, nCars, 1, nTicks]), [1, 1, nSims, 1]);
DCL_err  = [sqrt(DCL_diff(1,:,:,:).^2 + DCL_diff(2,:,:,:).^2); DCL_diff(3,:,:,:,:)];
DCL_rmse = reshape(RMSE(DCL_err), [2, nSims]); 

% 
% GSF_MAP_diff = GSF_MAP_x(1:3,:,:,:) - repmat(reshape(x_truth(1:3,:,:), [3, nCars, 1, nTicks]), [1, 1, nSims, 1]);
% GSF_MAP_err  = [sqrt(GSF_MAP_diff(1,:,:,:).^2 + GSF_MAP_diff(2,:,:,:).^2); GSF_MAP_diff(3,:,:,:,:)];
% GSF_MAP_rmse = reshape(RMSE(GSF_MAP_err), [2, nSims]); 
% 
% 
% m = zeros(3*nCars,1);
% for i = 1:nCars
%     m(i*3-2) = i*6-5;
%     m(i*3-1) = i*6-4;
%     m(i*3-0) = i*6-3;
% end
% CKF_diff = CKF_x(m,:,:) - repmat(reshape(x_truth(1:3,:,:), [3*nCars, 1, nTicks]), [1, nSims, 1]);
% CKF_diff = reshape(CKF_diff, [3, nCars, nSims, nTicks]);
% CKF_err  = [sqrt(CKF_diff(1,:,:,:).^2 + CKF_diff(2,:,:,:).^2); CKF_diff(3,:,:,:,:)];
% CKF_rmse = reshape(RMSE(reshape(CKF_err, [2,nCars,nSims,nTicks])), [2, nSims]);

clear EKF_diff_with_gps EKF_err_with_gps;
clear EKF_diff_no_gps EKF_err_no_gps;
clear DCL_diff DCL_err;
% clear CKF_diff CKF_err;
% clear GSF_diff GSF_err;
% clear GSF_MAP_diff GSF_MAP_err;


function err = RMSE(x)
    m = size(x,1);
    n = size(x,3);
    
    N = size(x,2) * size(x,4);
    err = zeros(m, n);
    
    for i = 1:m
        for j = 1:n
            err(i,j) = sqrt( sum(x(i,:,j,:).^2,[2,4]) / N);
        end
    end
end
